package datastructures.heaps.heapconverter;
...
public class HeapConverter {
    ...
    public int[] transform() {
        // we do not have to consider leaf nodes (that's why the last node we have to consider has index (length-2)/2
        for (int i = ((heap.length -2)/2); i >= 0; i--) {
            // we "heapify" all the internal nodes: we check whether the parent is smaller than the children
            // if not: then we swap the nodes accordingly
            heapify(i);
        }
